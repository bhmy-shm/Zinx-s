...
func main(){
	...
	for {
		//发送封包的message数据
		dp := znet.NewDataPack()
		binaryMsg,err:=dp.Pack(znet.NewMessagePackage(1,[]byte("ZinxV0.9 client 1 Test Message")))
		if err != nil{
...
func SendHearBeatResponse(dp ziface.IDataPack,conn net.Conn){
	lock.Lock()
	defer lock.Unlock()
